...
public class CommonObjects {
	...
	//get values by using element.getattribute("value")
		...
		public void expandCommentSection() throws Exception {
			if(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(commentsSubHdr,"aria-expanded").equalsIgnoreCase("false")) {
			new ElementUtil(driver,ConfigReader.log).scrollToViewAndClickElement(commentsSubHdr);
			}
		}	
		public void waitForSpinner() {
			new ElementUtil(driver,ConfigReader.log).waitUntilElementInvisible(loadSpinner, Duration.ofSeconds(150));
		}
		public void enterComments(String commentsValue) {
			new ElementUtil(driver,ConfigReader.log).webEditTxtChange(commentTxt,commentsValue);
		}
		public void selectaccesslevel(String accessLevelValue) {
			new ElementUtil(driver,ConfigReader.log).selectFromDropdownByVisibleText(accessLeveldd,accessLevelValue);
		}
		public void checkDeleteAllowed() {
			new ElementUtil(driver,ConfigReader.log).webCheckON(deleteAllowedCommentChk);
		}
		public void clickAddOrUpdateComment() {
			new ElementUtil(driver,ConfigReader.log).clickElement(addorUpdateCommentBtn);
		}
		public void clickClearComment() {
			new ElementUtil(driver,ConfigReader.log).clickElement(refreshCommentBtn);
		}
		public void clickProceed() throws InterruptedException {
			new ElementUtil(driver,ConfigReader.log).clickElement(proceedBtn);
			//new ElementUtil(driver,ConfigReader.log).sleepTime(2000);
		}
		public void ClickConfirmCancel() {
			new ElementUtil(driver,ConfigReader.log).clickElement(confirmCancelBtn);
		}
		public void clickRefresh() {
			new ElementUtil(driver,ConfigReader.log).clickElement(refershBtn);
		}
		public void clickQuit() {
			new ElementUtil(driver,ConfigReader.log).waitUntilElementClickable(quitBtn);
			new ElementUtil(driver,ConfigReader.log).clickElement(quitBtn);
		}
		public void clickBack() {
			new ElementUtil(driver,ConfigReader.log).clickElement(backBtn);
		}
		public void clickCancelBtn() {
			new ElementUtil(driver,ConfigReader.log).clickElement(cancelBtn);
		}
		public void clickDoneBtn() throws Exception {
			new ElementUtil(driver,ConfigReader.log).clickElementIgnoreStaleElementReferenceException(doneBtn);
		}
		...
		public void validateInfoMessage(String inforMsg) {
			new ElementUtil(driver,ConfigReader.log).highlightElement(driver, informationMessage);
			if(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(informationMessage).equalsIgnoreCase(inforMsg)) {
			assert true;
			}
		}
	public ArrayList<String> validateInfoMsgs() {
		...
		if(!(errorMsgsList.isEmpty())) {
			for(WebElement ele:errorMsgsList) {
				infoMsgsArray.add(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(ele));	
				new ElementUtil(driver,ConfigReader.log).highlightElement(driver, ele);
			}
		}
	...
	public String fetchErrorMessage(String ExpectederrorMessage) {
		...
		for(WebElement ele:errorMsgsList) {
			actualErrorMessage	=new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(ele);
