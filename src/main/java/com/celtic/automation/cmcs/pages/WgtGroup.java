...
public class WgtGroup {
	...
	public void clickHandImg() {
		if(new ElementUtil(driver,ConfigReader.log).isPresentAndDisplayed(fleetGridFirstHandimg)) {
			new ElementUtil(driver,ConfigReader.log).clickElement(fleetGridFirstHandimg);
		}	
	}

	public void clickAddWeightGroup() {
		if(new ElementUtil(driver,ConfigReader.log).isPresentAndDisplayed(addWeightGroupBtn)) {
			new ElementUtil(driver,ConfigReader.log).clickElement(addWeightGroupBtn);
		}
	}

	public String fetchJurWithDiffWeights() {
		return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(jurWithWeight);
	}
	...
	public ArrayList<String> fetchTableHeader() {
		new ElementUtil(driver,ConfigReader.log).waitUntilElementsVisible(tableHeader);
		ArrayList<String>headersArray =new ArrayList<String>();
		for(int i=0;i<tableHeader.size();i++) {
			headersArray.add(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(tableHeader.get(i)));
		}
		return headersArray;
	}

	public ArrayList<String> fetchTableRowData() {
		new ElementUtil(driver,ConfigReader.log).waitUntilElementsVisible(weightTableRows);
		ArrayList<String>rowDataArray =new ArrayList<String>();
		for(int i=0;i<weightTableRows.size();i++) {
			rowDataArray.add(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(weightTableRows.get(i)));
		}
		return rowDataArray;
	}

	public String weightValidateMessage() {
		new ElementUtil(driver,ConfigReader.log).highlightElement(driver, weightVerificationMsg);
		return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(weightVerificationMsg);
	}	
