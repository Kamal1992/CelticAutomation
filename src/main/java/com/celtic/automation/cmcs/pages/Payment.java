...
//	@FindBy(xpath="//select[contains(@id,'PayType') and contains(@id,'"+i+"')]") List<WebElement> Payment_PaymentTypegriddd;
	...
	public void selectElectronicDeliverytype(String electronicdeliverytypevalue) {
		new ElementUtil(driver,ConfigReader.log).selectFromDropdownByVisibleText(paymentElectronicDeliveryTypedd, electronicdeliverytypevalue);
	}
	...
	public String fetchRemainingBalance() {
		new ElementUtil(driver,ConfigReader.log).waitUntilElementVisible(paymentRemainingBalanceTxt);
		return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentRemainingBalanceTxt,"value");
	}
	public void clickPayNow() {
	new ElementUtil(driver,ConfigReader.log).clickElement(paymentPayBtn);
}
public void selectPaymentType(int i,String paymentTypeValue) {
	...
	new ElementUtil(driver,ConfigReader.log).selectFromDropdownByVisibleText(Payment_PaymentType, paymentTypeValue); //Cash,Check,Certified Check,E-check,Credit Card,Wire Transfer,EFT,Enterprise System Credit,IRP System Credit,IFTA System Credit,OPA System Credit,IFTA Bond
}
public void enterpaymentNoBasedonType(int i,String paymentNumberValue) {
	...
	WebElement Payment_PaymentType=driver.findElement(By.xpath("//select[contains(@id,'PayType') and contains(@id,'"+i+"')]"));
	if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Check")) {
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentNumbertxt, paymentNumberValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Certified Check")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentNumbertxt, paymentNumberValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Wire Transfer")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentNumbertxt, paymentNumberValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("EFT")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentNumbertxt, paymentNumberValue);
	}
...
public void enterPaymentAmountBasedonType(int i,String paymentAmountValue) {
	...
	WebElement Payment_PaymentType=driver.findElement(By.xpath("//select[contains(@id,'PayType') and contains(@id,'"+i+"')]"));
	if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Cash")) {
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Check")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Certified Check")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	...
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("Enterprise System Credit")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("IRP System Credit")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("IFTA System Credit")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("OPA System Credit")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
	else if(new ElementUtil(driver,ConfigReader.log).FetchDropdownSelectedValue(Payment_PaymentType).trim().equalsIgnoreCase("IFTA Bond")){
		new ElementUtil(driver,ConfigReader.log).webEditTxt(Payment_PaymentAmount, paymentAmountValue);
	}
}
public void validatemsg(String msgValue) {
	...
	//new ElementUtil(driver,ConfigReader.log).highlightElement(driver, validationMessage);
public void clickDelete() {
	new ElementUtil(driver,ConfigReader.log).clickElement(deleteBtn);
}
public void clickAdd() {
	new ElementUtil(driver,ConfigReader.log).clickElement(addBtn);
}
public void clickCreditCard() {
	new ElementUtil(driver,ConfigReader.log).clickElement(creditCardBtn);
}
public void clickECheck() {
	new ElementUtil(driver,ConfigReader.log).clickElement(eCheckBtn);
}
...
public void selectOverPaymentRadioButton(String paymentRdValue) {
	for(WebElement rd:forOverPaymentrdList) {
		if(rd.getAttribute("value").equalsIgnoreCase(paymentRdValue)) {
			new ElementUtil(driver,ConfigReader.log).clickElement(rd);
		}
	}
}

public void selectPaymentReceipt(String paymentReceiptddValue) {
	new ElementUtil(driver,ConfigReader.log).selectFromDropdownByVisibleText(paymentPaymentReceiptdd, paymentReceiptddValue); //D - PDF,E - EMAIL,F - FAX,P - PRINT
}

//Fetch Values
public String fetchMCECustomerIdLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentPayerMCECustomerIdLbl);
}
public String fetchMCECustomerId() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentPayerMCECustomerIdTxt,"value");
}
public String fetchLegalNameLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentLegalNameLbl);
}
public String fetchLegalName() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentLegalNameTxt,"value");
}
public String fetchInvoiceDateLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentDBANameLbl);
}
public String fetchInvoiceDate() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentDBANameTxt,"value");
}
...
public String fetchIRPSystemCredit() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentIRPSystemCreditTxt,"value");
}
public String fetchIftaSystemCreditLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentIftaSystemCreditLbl);
}
public String fetchIftaSystemCredit() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentIftaSystemCreditTxt,"value");
}
public String fetchOpaSystemCreditLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentOpaSystemCreditLbl);
}
public String fetchOpaSystemCredit() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithattribute(paymentOpaSystemCreditTxt,"value");
}
public ArrayList<String> fetchTableHeaders() {
	...
	for(int i=0;i<paymentTableHeaderRow.size();i++) {
		headersArray.add(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentTableHeaderRow.get(i)));
	}
	return headersArray;
}
...
public ArrayList<String> fetchTableInvoiceDetails() {
	...
	for(int i=0;i<paymentInvoiceRow.size();i++) {
		invoiceArray.add(new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentInvoiceRow.get(i)));
	}
	return invoiceArray;
}
...
public String fetchTotalAmountDueLbl() {
	return new ElementUtil(driver,ConfigReader.log).FetchTextBoxValuewithText(paymentTotalAmountDueLbl);
}
